...
def main():
    ...
    Pontos = list(list())
    #xOrigem = rospy.get_param("$(find smb_gazebo)/config/xOrigem")
    #yOrigem = rospy.get_param("$(find smb_gazebo)/config/yOrigem")
    #xDestino = rospy.get_param("$(find smb_gazebo)/config/xDestino")
    #yDestino = rospy.get_param("$(find smb_gazebo)/config/yDestino")
    #Animacao = rospy.get_param("$(find smb_gazebo)/config/Animacao")
    ...
    #xOrigem = rospy.get_param("xOrigem")
    #yOrigem = rospy.get_param("yOrigem")
    
    #xDestino = rospy.get_param("xDestino")
    #yDestino = rospy.get_param("yDestino")
    
    #NomeMapa = rospy.get_param("NomeMapa")                      # Nome do a ser percorrido
    #Animacao = rospy.get_param("Animacao")
    ...
    TamM = min(int(Dim[0]), int(Dim[1]))
    TamR = 29.186   # Tamanho real em [m]
    
    # Localização dos obstáculos
    ox, oy = [], []
    
    M = np.loadtxt(NomeMapa, delimiter=',', skiprows= ErroDim*TamM + 4).reshape(TamM, TamM)  # Lê matriz externa, mapa.yaml reduzido por conta do processamento
    
    for i in range(len(M)):
        for j in range(len(M[0])):
            if M[-1-i][j] == 0:         # 0 é a cor preta no .pgm lido como texto, ou seja, obstáculo
                oy.append(i)
                ox.append(j)

    TamM += 1
    # Pontos iniciais convertidos de [m] para pxls
    sx = (xOrigem*TamM)/TamR        # X inicial
    sy = (yOrigem*TamM)/TamR        # Y inicial
    gx = (xDestino*TamM)/TamR       # X final
    gy = (yDestino*TamM)/TamR       # Y final
    grid_size = 3                  # Distâncias entre as esquinas
    robot_radius = (1*TamM)/TamR    # Tamanho do robô
